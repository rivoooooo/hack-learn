...
> Simplify building modern BPF programs This package contains bpd-linker which can be used to statically link multiple BPF object files together and optionally perform optimizations needed to target older kernels. It operates on LLVM bitcode…

> **功能分类**：通用工具 ｜ **Kali 包**：`bpf-linker` ｜ **官方文档**：<https://www.kali.org/tools/bpf-linker/>
...
| 源码仓库 | <https://gitlab.com/kalilinux/packages/bpf-linker> |
| 包追踪 | <https://pkg.kali.org/pkg/bpf-linker> |
...
- [ ] **1.** **装好并确认版本** —— `sudo apt install bpf-linker`，再执行 `bpf-linker --version` 2>/dev/null || `bpf-linker -V`
- [ ] **2.** **读官方帮助** —— `bpf-linker -h`，需要细节时 `man bpf-linker`
- [ ] **3.** **在自有靶场跑最小用例** —— 官方给出的调用形式是：`Usage: bpf-linker [OPTIONS] --output <OUTPUT> <INPUTS>...`
- [ ] **4.** **对照官方文档确认参数语义** —— <https://www.kali.org/tools/bpf-linker/>，重点核对上一步用到的每个参数
...
- Kali 官方工具页：<https://www.kali.org/tools/bpf-linker/>

---

## 来源

- 工具元数据与用法示例、help 原文均抓取自 <https://www.kali.org/tools/bpf-linker/>（同步日期：2026-09-15）
