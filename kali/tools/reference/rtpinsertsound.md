...
> Inserts audio into a specified stream A tool to insert audio into a specified audio (i.e. RTP) stream was created in the August - September 2006 timeframe. The tool is named rtpinsertsound. It was tested on a Linux Red Hat Fedora Core 4 pl…

> **功能分类**：漏洞分析 ｜ **Kali 包**：`rtpinsertsound` ｜ **官方文档**：<https://www.kali.org/tools/rtpinsertsound/>
...
| 版本 | 3.0 |
| 架构 | any |
| 可执行命令 | `rtpinsertsound` |
| 依赖 | `libc6`、`libnet9`、`libpcap0.8t64` |
| 安装体积 | 239 KB |
| 官网 | <http://www.hackingvoip.com/sec_tools.html> |
| 源码仓库 | <https://gitlab.com/kalilinux/packages/rtpinsertsound> |
| 包追踪 | <https://pkg.kali.org/pkg/rtpinsertsound> |
...
```bash
rtpinsertsound -h          # 查看用法
man rtpinsertsound         # 查看手册
```
...
- [ ] **1.** **装好并确认版本** —— `sudo apt install rtpinsertsound`，再执行 `rtpinsertsound --version` 2>/dev/null || `rtpinsertsound -V`
- [ ] **2.** **读官方帮助** —— `rtpinsertsound -h`，需要细节时 `man rtpinsertsound`
- [ ] **3.** **在自有靶场跑最小用例** —— 官方给出的调用形式是：`Usage:`
- [ ] **4.** **对照官方文档确认参数语义** —— <https://www.kali.org/tools/rtpinsertsound/>，重点核对上一步用到的每个参数
...
- Kali 官方工具页：<https://www.kali.org/tools/rtpinsertsound/>

---

## 来源

- 工具元数据与用法示例、help 原文均抓取自 <https://www.kali.org/tools/rtpinsertsound/>（同步日期：2026-09-15）
