...
> The Network Mapper Nmap is a utility for network exploration or security auditing. It supports ping scanning (determine which hosts are up), many port scanning techniques, version detection (determine service protocols and application vers…

> **功能分类**：信息搜集 ｜ **Kali 包**：`nmap` ｜ **官方文档**：<https://www.kali.org/tools/nmap/>
...
| 版本 | 7.99 |
| 架构 | any |
| 可执行命令 | `ncat`、`ndiff`、`nmap`、`nping`、`nmap-common`、`zenmap` |
| 依赖 | `libc6`、`libgcc-s1`、`liblinear4`、`liblua5.4-0`、`libpcap0.8t64`、`libpcre2-8-0`、`libssh2-1t64`、`libssl3t64`、`libstdc++6`、`nmap-common`、`zlib1g` |
| 安装体积 | 4.70 MB |
| 官网 | <https://nmap.org/> |
| 源码仓库 | <https://gitlab.com/kalilinux/packages/nmap> |
| 包追踪 | <https://pkg.kali.org/pkg/nmap> |
...
ndiff Usage Example
Compare yesterday’s port scan (
yesterday.xml
) with the scan from today (
today.xml
):
```
...
Be verbose (
-v
), running /bin/bash on connect (
–exec “/bin/bash”
), only allowing 1 IP address (
–allow 192.168.1.123
), listen on TCP port 4444 (
-l 4444
), and keep the listener open on disconnect (
–keep-open
):
```
...
- [ ] **1.** **装好并确认版本** —— `sudo apt install nmap`，再执行 `ncat --version` 2>/dev/null || `ncat -V`
- [ ] **2.** **读官方帮助** —— `ncat -h`，需要细节时 `man ncat`
- [ ] **3.** **在自有靶场跑最小用例** —— 官方给出的调用形式是：`Usage: ncat [options] [hostname] [port]`
- [ ] **4.** **对照官方文档确认参数语义** —— <https://www.kali.org/tools/nmap/>，重点核对上一步用到的每个参数
...
- Kali 官方工具页：<https://www.kali.org/tools/nmap/>

---

## 来源

- 工具元数据与用法示例、help 原文均抓取自 <https://www.kali.org/tools/nmap/>（同步日期：2026-09-15）
