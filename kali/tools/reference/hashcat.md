...
> World’s fastest and most advanced password recovery utility Hashcat supports five unique modes of attack for over 300 highly-optimized hashing algorithms. hashcat currently supports CPUs, GPUs, and other hardware accelerators on Linux, and…

> **功能分类**：口令攻击 ｜ **Kali 包**：`hashcat` ｜ **官方文档**：<https://www.kali.org/tools/hashcat/>
...
| 版本 | 7.1.2 |
| 架构 | kfreebsd-any |
| 可执行命令 | `hashcat`、`hashcat-data` |
| 依赖 | `hashcat-data`、`libc6`、`libminizip1t64`、`libxxhash0` |
| 安装体积 | 119.52 MB |
| 官网 | <https://hashcat.net/hashcat/> |
| 源码仓库 | <https://salsa.debian.org/pkg-security-team/hashcat> |
| 包追踪 | <https://pkg.kali.org/pkg/hashcat> |
...
Use md5crypt mode (
-m 500
) to cracking the sample hash (
example500.hash
) with the provided wordlist (
...
- [ ] **1.** **装好并确认版本** —— `sudo apt install hashcat`，再执行 `hashcat --version` 2>/dev/null || `hashcat -V`
- [ ] **2.** **读官方帮助** —— `hashcat -h`，需要细节时 `man hashcat`
- [ ] **3.** **在自有靶场跑最小用例** —— 官方给出的调用形式是：`Usage: hashcat [options]... hash|hashfile|hccapxfile [dictionary|mask|directory]...`
- [ ] **4.** **对照官方文档确认参数语义** —— <https://www.kali.org/tools/hashcat/>，重点核对上一步用到的每个参数
...
- Kali 官方工具页：<https://www.kali.org/tools/hashcat/>

---

## 来源

- 工具元数据与用法示例、help 原文均抓取自 <https://www.kali.org/tools/hashcat/>（同步日期：2026-09-15）
