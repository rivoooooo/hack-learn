...
> Recover files on disks with damaged sectors recoverdm recover disks with bad sectors. You can recover files as well complete devices. In case it finds sectors which simply cannot be recovered, it writes an empty sector to the output file a…

> **功能分类**：数字取证 ｜ **Kali 包**：`recoverdm` ｜ **官方文档**：<https://www.kali.org/tools/recoverdm/>
...
| 版本 | 0.20 |
| 架构 | any |
| 可执行命令 | `recoverdm`、`mergebad` |
| 依赖 | `libc6`、`mergebad` |
| 安装体积 | 64 KB |
| 官网 | <https://www.vanheusden.com/recoverdm> |
| 源码仓库 | <https://salsa.debian.org/pkg-security-team/recoverdm> |
| 包追踪 | <https://pkg.kali.org/pkg/recoverdm> |
...
- [ ] **1.** **装好并确认版本** —— `sudo apt install recoverdm`，再执行 `recoverdm --version` 2>/dev/null || `recoverdm -V`
- [ ] **2.** **读官方帮助** —— `recoverdm -h`，需要细节时 `man recoverdm`
- [ ] **3.** **在自有靶场跑最小用例** —— 官方给出的调用形式是：`Usage: recoverdm -t <type> -i <file/device-in> -o <fileout> [-l <sectorsfile>] [-n # retries]`
- [ ] **4.** **对照官方文档确认参数语义** —— <https://www.kali.org/tools/recoverdm/>，重点核对上一步用到的每个参数
...
- Kali 官方工具页：<https://www.kali.org/tools/recoverdm/>

---

## 来源

- 工具元数据与用法示例、help 原文均抓取自 <https://www.kali.org/tools/recoverdm/>（同步日期：2026-09-15）
